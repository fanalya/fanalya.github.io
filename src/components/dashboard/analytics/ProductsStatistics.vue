...
<template>
  <TCard
    ...
    class="order-11 col-span-12 2xl:order-1 card 2xl:col-span-12"
  >
        ...
        <template #status="{ value }">
          <TLabel :color="value.status === 'Active' ? 'green' : 'red'">
            <template v-if="value.status === 'Active'">
              <CheckCircle2 class="size-3 ltr:mr-1 rtl:ml-1" />
              {{ value.status }}
            </template>
            <template v-else>
              <XCircle class="size-3 ltr:mr-1 rtl:ml-1" /> {{ value.status }}
            </template>
          </TLabel>
        </template>
      </TBasicTable>
